...
class AdminTest(TestCase):
    ...
    def test_admin_controls(self):
        ...
        # Add Ontologies
        with open("./tests/data/ontology/test.po_table.txt", "rb") as po_data,\
             open("./tests/data/ontology/test.peco_table.txt", "rb") as peco_data :
            payload = {"peco": peco_data, "po": po_data}

            response = self.client.post(
                "/admin_controls/add/ontology",
                data=payload,
                follow_redirects=True,
            ...
            self.assert200(response)
            self.assertTrue(
                "Ontologies data added." in response.data.decode("utf-8")
            )  # Check for flash message

        # Add functional data
        with open("./tests/data/test_go.obo", "rb") as go_data, open(
            "./tests/data/test_interpro.xml", "rb"
        ) as ip_data, open("./tests/data/test_cazyme.txt", "rb") as cazyme_data:
            payload = {"go": go_data, "interpro": ip_data, "cazyme": cazyme_data}
            ...
            self.assertTrue(
                "CAZYme data added." in response.data.decode("utf-8")
            )

        # Add Species
        with open("./tests/data/test.cds.fasta", "rb") as fasta_cds_data,\
            open("./tests/data/test.rna.fasta", "rb") as fasta_rna_data:
            payload = {
                "code": "tst",
                "name": "Test species",
                "data_type": "genome",
                "color": "000000",
                "highlight": "DDDDDD",
                "fasta_cds": fasta_cds_data,
                "fasta_rna": fasta_rna_data,
