...
class BuildTest(TestCase):
    ...
    def setUp(self):
        ...
        Sequence.add_from_fasta("./tests/data/test.rna.fasta", s.id, sequence_type='RNA')
        Sequence.add_descriptions("./tests/data/test.descriptions.txt", s.id)

        PlantOntology.add_tabular_po("./tests/data/ontology/test.po_table.txt", empty=True, compressed=False)
        PlantExperimentalConditionsOntology.add_tabular_peco("./tests/data/ontology/test.peco_table.txt", empty=True, compressed=False)

        XRef.add_xref_genes_from_file(s.id, "./tests/data/test.xref.txt")
        GO.add_from_obo("./tests/data/test_go.obo")
        GO.add_go_from_tab(
            "./tests/data/functional_data/test.go.txt",
            s.id,
        ...
        Interpro.add_interpro_from_interproscan(
            "./tests/data/functional_data/test.interpro.txt", s.id
        )

        ExpressionProfile.add_profile_from_lstrap(
            "./tests/data/expression/test.tpm.matrix.txt",
        ...
        ExpressionSpecificityMethod.calculate_specificities(
            s.id, s.name + " condition specific profiles", False
        )

        GeneFamily.add_families_from_mcl(
            "./tests/data/comparative_data/test.families.mcl.txt", "Fake Families"
        )
    ...
    def test_build(self):
        ...
        test_go_association = test_sequence.go_associations.filter_by(
            evidence=None
        ).first()
        test_interpro = test_sequence.interpro_domains.filter_by(
            label="IPR000001"
        ).first()
        ...
        self.assertEqual(len(s.sequences.all()), 6)  # Check if all genes are added
        ...
        self.assertEqual(
            test_sequence.aliases, "BRCA2"
        )  # Check if alias is added and correct
        ...
        self.assertEqual(
            test_go.description, '"Third label"'
        )  # Check if go description is parsed
        self.assertEqual(
            test_go_association.go.label, "GO:0000001"
        )  # Check if go parent is added

        self.assertEqual(
            test_interpro.label, "IPR000001"
        )  # Check if Interpro domain is added
        self.assertTrue(
            "Kringle" in test_interpro.description
        )  # Check if description is added
        self.assertEqual(
            test_interpro.species_counts["tst"], 1
        )  # Check if species profiles are generated

        self.assertEqual(
            test_profile.sequence.name, "Gene01"
        )  # Check if profile is linked with gene
        self.assertEqual(test_profile.probe, "Gene01")  # Check if probe is set
        self.assertTrue(
            "order" in test_profile_data.keys()
        )  # Check if profile data contains order
        self.assertTrue(
            "data" in test_profile_data.keys()
        )  # Check if profile data contains data
        self.assertTrue(
            "colors" in test_profile_data.keys()
        )  # Check if profile data contains colors

        self.assertEqual(
            test_network_data[0]["gene_name"], "Gene02"
        )  # Check if network contains required fields
        self.assertEqual(
            test_network_data[0]["probe_name"], "Gene02"
        )  # Check if network contains required fields
        self.assertEqual(
            test_network_data[0]["link_pcc"], 0.71
        )  # Check if network contains required fields
        self.assertEqual(
            test_network_data[0]["link_score"], 0
        )  # Check if network contains required fields

        self.assertNotEqual(cluster_sequence, None)  # Check if gene is in cluster

        self.assertEqual(
            test_profile.specificities.first().condition, "Tissue 03"
        )  # Check if SPM worked
        self.assertAlmostEqual(
            test_profile.specificities.first().score, 0.62, places=2
        )  # Check if SPM score is correct
        self.assertAlmostEqual(
            test_profile.specificities.first().entropy, 1.58, places=2
        )  # Check if entropy is correct
        self.assertAlmostEqual(
            test_profile.specificities.first().tau, 0.11, places=2
        )  # Check if tau is correct

        self.assertEqual(
            len(test_family.sequences.all()), 2
        )  # Check if gene family contains 2 genes
