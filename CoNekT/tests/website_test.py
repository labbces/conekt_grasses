...
@pytest.mark.website
@pytest.mark.db
class TestSequenceOntologyRoutes:
    """Tests for Sequence Ontology (SO) routes."""

    def test_so_overview_redirect(self, client):
        """Tests redirect of the overview of Sequence Ontology."""
        response = client.get("/so/")
        assert response.status_code == 302

    def test_so_redirect(self, client, full_test_data):
        """Tests redirect from find to view."""
        if 'sequence_ontologies' in full_test_data and len(full_test_data['sequence_ontologies']) > 0:
            so_term = full_test_data['sequence_ontologies'][0]
            response = client.get(f"/so/find/{so_term.so_id}")
            assert response.status_code == 302

    def test_so_view(self, client, full_test_data):
        """Tests view of SO term."""
        if 'sequence_ontologies' in full_test_data and len(full_test_data['sequence_ontologies']) > 0:
            so_term = full_test_data['sequence_ontologies'][0]
            response = client.get(f"/so/view/{so_term.id}")
            assert response.status_code == 200
            assert so_term.name.encode() in response.data

    def test_so_sequences(self, client, full_test_data):
        """Tests pagination of sequences of SO term."""
        if 'sequence_ontologies' in full_test_data and len(full_test_data['sequence_ontologies']) > 0:
            so_term = full_test_data['sequence_ontologies'][0]
            response = client.get(f"/so/sequences/{so_term.id}/")
            assert response.status_code == 200

    def test_so_sequences_table(self, client, full_test_data):
        """Tests CSV table of sequences of SO term."""
        if 'sequence_ontologies' in full_test_data and len(full_test_data['sequence_ontologies']) > 0:
            so_term = full_test_data['sequence_ontologies'][0]
            response = client.get(f"/so/sequences/table/{so_term.id}")
            assert response.status_code == 200
            assert response.mimetype == "application/json"

    def test_so_json_species(self, client, full_test_data):
        """Tests JSON distribution of species of SO term."""
        if 'sequence_ontologies' in full_test_data and len(full_test_data['sequence_ontologies']) > 0:
            so_term = full_test_data['sequence_ontologies'][0]
            response = client.get(f"/so/json/{so_term.id}")
            assert response.status_code == 200
            
            data = json.loads(response.data.decode("utf-8"))
            # The JSON returns a nested structure with 'data' containing datasets and labels
            assert "data" in data
            assert "datasets" in data["data"]
            assert "labels" in data["data"]
