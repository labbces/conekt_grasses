...
@species.route('/transposable_elements/<species_id>/')
@species.route('/transposable_elements/<species_id>/<int:page>')
@cache.cached()
def species_transposable_elements(species_id, page=1):
	"""
	Returns a table with transposable elements from the selected species

	:param species_id: Internal ID of the species
	:param page: Page number
	"""
	sequences = Species.query.get(species_id).sequences.filter(Sequence.type == 'TE').paginate(page,
																 g.page_items,
																 False).items
...
#stream te fasta -----------------------------------------------
@species.route('/stream/transposable_elements/<species_id>')
def species_stream_te(species_id):
	"""
	Generates a fasta file with all RNA sequences for a given species. However this is send as a streaming
	response (and seems to bring up the download dialog a few seconds faster)
