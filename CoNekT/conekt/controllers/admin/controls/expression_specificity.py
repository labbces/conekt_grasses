...
from conekt.forms.admin.add_expression_specificity import AddConditionSpecificityForm, AddTissueSpecificityForm, AddPOSpecificityForm
...
@admin_controls.route('/add/po_specificity', methods=['POST'])
@admin_required
def add_po_specificity():
    form = AddPOSpecificityForm(request.form)
    form.populate_species()
    ...
    if request.method == 'POST' and form.validate():
        ...
        if file != b'':
            ...
            for d in data:
                condition, po, color = d.split("\t")

                conditions.append(condition)

                condition_po[condition] = po
            ...
            specificity_method_id = ExpressionSpecificityMethod.calculate_tissue_specificities(species_id, description,
                                                                                               ...
                                                                                               use_max=True,
                                                                                               remove_background=False)
            ConditionTissue.add(species_id, condition_po, order, colors, specificity_method_id)
