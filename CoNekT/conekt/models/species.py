...
class Species(db.Model):
	...
	non_coding_seq_count = db.Column(db.Integer)
	te_count = db.Column(db.Integer)
	...
	@property
	def has_te_class(self):
		from conekt.models.sequences import Sequence
		from conekt.models.relationships.sequence_te_class import SequenceTEClassAssociation

		te_class = SequenceTEClassAssociation.query.join(Sequence, Sequence.id == SequenceTEClassAssociation.sequence_id).filter(Sequence.species_id == self.id).first()

		if te_class is not None:
			return True
		else:
			return False
